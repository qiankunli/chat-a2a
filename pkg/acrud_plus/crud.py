...
from pkg.acrud_plus.errors import MultipleResultsError
from pkg.acrud_plus.types import CreateSchema, Model, UpdateSchema
from pkg.acrud_plus.utils import count


class ACRUDPlus(Generic[Model]):
    ...
    async def create_model(
        ...
        :return:
        """
        ins = self.do_to_model(obj, **kwargs)
        session.add(ins)
        if commit:
    ...
    async def create_models(
        ...
        ins_list = []
        for ins in obj:
            ins_list.append(self.do_to_model(ins))
        session.add_all(ins_list)
        if commit:
            await session.commit()
        return ins_list

    async def select_model_by_id(self, session: AsyncSession, pk: str) -> Model | None:
        """
        Query by ID
    ...
    async def select_models_by_column(self, session: AsyncSession, **kwargs) -> Sequence[Model]:
        """
        Query by column
    ...
    async def update_model(
        ...
        if isinstance(obj, dict):
            ...
            instance_data = self.do_to_model(obj)
    ...
    async def delete_model_by_column(
        ...
        if logical_deletion:
            ...
            stmt = delete(self.model).where(*filters)  # type:ignore
        result = await session.execute(stmt)
        if commit:
            await session.commit()
        return result.rowcount  # type: ignore
