...
from agent.gate_event import (
    ...
    HeaderEvent,
    MessageEvent,
    StartEvent, AgentInputRequiredEvent,
)
...
class ChatService:
    ...
    async def stream_chat(self, request: ChatRequest) -> AsyncGenerator[Event, None]:
        ...
        # 如果存在 input_required，则复用input_required message 对应的task id（下游对应的agent 还处于input_required 状态）
        task_id = conversation.input_required_task_id or uuid7_hex()
        ...
        # 如果存在 input_required，则汇总同task_id 下的agent 执行记录，便于plan理解用户最新query
        agent_history = conversation.agent_history(task_id)
        context = GateContext(conversation_id=conversation.id, task_id=task_id, chat_history=conversation.chat_history,
                              agent_history=agent_history,
                              agent_configs=[A2AConfig.from_agent_desc(agent_desc) for agent_desc in agent_desc_list])
        logger.info(f"try chat query {request.query},task_id {task_id}, "
                    f"agent_history {agent_history} in conversation {conversation.id}")
        user_request = UserRequest(query=request.query)
        await self.message_service.add_user(conversation.id, task_id, request.query)

        async def response_stream() -> AsyncGenerator[Event, None]:
            yield StartEvent(conversation_id=conversation.id, task_id=task_id)
            ...
            try:
                async for evt in gate_agent.astream_event(context, user_request):
                    if isinstance(evt, MessageEvent):
                        ...
                        await self.message_service.add_agent_query(conversation.id, task_id,
                                                                   query=evt.chunk,
                                                                   from_agent=evt.name)
                    elif isinstance(evt, AnswerEvent):
                        answer += evt.chunk
                    elif isinstance(evt, AgentEndEvent):
                        await self.message_service.add_agent_answer(conversation.id, task_id,
                                                                    answer=group_stream_content[evt.name],
                                                                    from_agent=evt.name)
                    elif isinstance(evt, AgentInputRequiredEvent):
                        await self.message_service.add_agent_input_required(conversation.id, task_id,
                                                                            require_input=evt.require_input,
                                                                            from_agent=evt.name)
                    elif isinstance(evt, AgentEndEvent):
                        await self.message_service.add_agent_error(conversation.id, task_id,
                                                                   from_agent=evt.name)
                    elif isinstance(evt, AnswerEndEvent):
                        await self.message_service.add_answer(conversation.id, task_id, answer=answer)
                    yield evt
